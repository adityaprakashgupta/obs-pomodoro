...
function useQueryParams(): TimerSettings & {
...
function App() {
  const { showPauseButton, twitchChannel, ...settings } = useQueryParams();
  const status = useTimer(settings);

  const handleTwitchCommand = (
    ...
    if (command === "timer" && flags.broadcaster) {
  ...
  // Initialize Twitch chat if channel is provided
  useTwitchChat({
    channel: twitchChannel || "",
    onCommand: handleTwitchCommand,
  });
      ...
      <div className="bg-gray-900/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-gray-800 flex items-center gap-6">
        <div className="flex items-center gap-3">
          {status.isPaused ? (
            ...
            <TimerReset
              className={`w-8 h-8 ${stateColors[status.timerState]}`}
            />
          )}
          <div>
            <h2
              className={`text-4xl font-bold font-mono ${
                stateColors[status.timerState]
              }`}
            >
              {formatTime(status.timeRemaining)}
            </h2>
            <p className="text-gray-400 text-sm">
              {stateLabels[status.timerState]}
            </p>
          </div>
        </div>
